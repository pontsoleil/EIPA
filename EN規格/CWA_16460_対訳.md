...
![with Service Provider](図/fig2.png)  
...
これらの各イベントの電子記録は通常、ERPシステムで作成される。 このレコードには、イベントに関連する値を直接含めることができる。 数量、またはコンテンツを提供または導出するためのマスターデータの参照。 価格。 監査証跡に寄与するのは、プロセス内の一連のイベントのこのレコードである。 監査証跡は、ERP外のドキュメントとERP内のトランザクションレコードで構成される。 たとえば、「発注」サイクルの監査証跡は、多くの場合、次の形式になる。  
...
取引相手がサービスプロバイダを使用する場合、これは、サービスプロバイダへの取引相手の納税義務または責任のアウトソーシングを意味するものではない。 したがって、サービスプロバイダーへの電子インボイスの処理過程のアウトソーシングは、取引相手が税務当局やその他の当局に非準拠であることの言い訳にはならない。 サービスプロバイダーの法的義務は、ほとんどの国で厳密に契約上である。 その結果、サービスプロバイダーの義務は、取引相手の責任に由来するだけである。 これは、本ガイドラインがサービスプロバイダーの責任を扱う方法でもある。  
...
多くの場合、電子インボイスは自動化された電子インボイスの処理過程で発行される。 プロセスのセキュリティ対策が侵害された場合、攻撃者は署名プロセスに偽のドキュメントを送信することに成功する可能性がある。 これが発生した場合、その後の検証でこれらのドキュメントを偽物として認識できることを保証するために、CEN作業部会合意書 CWA 15579（36）に指定されているような対策を実施する必要がある。
...
電子インボイスの内容に制限を加えることにより、さらに高いレベルの保護を実現できる。 スクリプトの使用を避け、アクティブなコードとスクリプトを含む電子インボイスを拒否することは、最初のステップとして適切である。 正しいドキュメント形式エンコーディングを選択すると、脆弱性をさらに大幅に減らすことができる。  
    ...
    a）オフィス文書やスプレッドシートなど、複雑な形式や数式機能を備えた編集可能な形式の使用は避ける。 複雑な式が悪意のあるものかどうかを見分けるのは不可能である。 編集可能なドキュメントの書式設定データを変更する機能は、特に乱用に対して脆弱になる。 Wordファイルプレゼンテーション（「shift + F9」）を「フリーズ」することが可能であっても、受信者は、電子インボイスを確定する前に（たとえば、署名する前に）このプレゼンテーションがフリーズされたかどうかを知ることができない。
    ...
    b）XMLやPDFなどの形式は、乱用されにくい。 PDF/Aは、使用をさらに制限し、フォントなどのすべての関連形式情報をドキュメントに含める必要があるため、一般的なPDFよりも望ましい。
...
電子インボイスのエラーが発生する可能性があり、取引相手との間、およびVAT管理者との間で正しいバランスの取れた会計に到達するためには修正が必要となる。  
...
取引の更正; クレジットノートや新しい請求書は、監査証跡および関連する在庫移動で明確に識別できる必要がある。  
...
税法、会計法、および加盟国の法律は、多くの場合、具体的な例を提供しておらず、利用可能なものは解釈の自由がある。 いくつかの状況の例はこの箇条の後半に示され、一部の税務当局はウェブサイトで説明情報を提供している。  
...
自動化された環境では、いくつかの理由でエラーが発生する可能性がある。
* 取引相手のアプリケーションシステムで使用可能な基本データが正しくないか、タイムリーに維持されていないため、予期しないエラーが発生する。 返品、新しい価格表、顧客割引条件、住所の変更、VAT番号の変更など、請求書でエラーが発生する。
* 注文に含まれる偶発的なプロモーションのためにトランザクションレベルで合意された特別な条件。ただし、売り手の電子インボイスには反映されていない。
* 売り手が電子インボイスで買い手と合意しなかった要求を手動で導入した。

#### 3.8.6.1 Rejecting an E-Invoice ( 3.8.6.1 電子インボイスの拒否 )

Art. 232 of Directive 2006/112/EC (1), as amended by 2010/45/EU (2), states: “The use of an electronic invoice shall be subject to acceptance by the recipient”. The acceptation mentioned in this article only reflects the technical requirements or the customer's ability to ensure the authenticity, integrity and legibility that might need to be agreed to receive electronic invoices and which do not exist for paper invoices. This may include any written acceptance, whether formal or not, or by tacit agreement through, for instance, the processing or payment of the received invoice.  

指令2006/112/EC（1）232条（2010/45/EU（2）によって修正）には、次のように記載されている。「電子インボイスの使用は、受信者による承認を受ける必要がある」 この条に記載されている承諾は、電子インボイスを受け取るために同意する必要があり、紙の請求書では存在しない、完全性、真正性、可読性を保証する技術要件または顧客の能力のみを反映している。 これには、正式かどうかや暗黙の合意によるかに関係なく、受領した電子インボイスの処理や支払いなどの書面による承諾が含まれる場合がある。  
...
ただし、取引相手は、電子インボイスを受け付けないことが技術的な理由によるものかビジネス上の理由によるものかを区別する必要がある。  
...
電子インボイスを交換するときに取引相手の間で合意が行われる場合、電子インボイスが技術的な理由で受け入れられたという事実は、電子インボイスがビジネス用途で受け入れられたことを自動的に意味するものではないことを言及する必要がある。  
...
技術的な理由で電子インボイスが受け入れられなかった場合でも、電子インボイスが受信されないからといって、自動的にビジネス上の理由で受け入れられないことを意味しない。  
...
電子インボイスが技術的な理由で受け入れられたが、電子インボイスに異議がある場合、電子インボイスはビジネス上の理由で拒否される可能性がある。 紛争は原則として、修正電子インボイスまたはクレジットノートの発行につながる。  
...
次のリストには、電子インボイスの拒否のビジネス上の理由の例が含まれている。
*  電子インボイスに記載されている誤った注文書（PO）/　注文書はキャンセルされた
*  電子インボイスはVAT要件に準拠していない：
  - 間違った内容。 VAT IDや将来の日付などの値
  - フィールドがない。 減税または0％の評価されたVATの理由を見逃している（現地の法律参照によるEU内貿易）
* 買い手と売り手の契約によっては、キャンセルが同意される場合があります。
  - 売り手への発注の電子インボイスに発注番号が含まれていない
  - 注文書と 電子インボイスの通貨が不一致
  - 電子インボイス金額が注文書の値より高いか、許容範囲外
  - 電子インボイス数量が発注数量より多いか、許容範囲外
  - 注文書と比較して許容単価の不一致/許容範囲外

The Customer should not modify any Invoice, including an incorrect or contested Invoice.  

買い手は、間違ったまたは係争中の電子インボイスであっても、電子インボイスを変更してはならない。
...
電子インボイスの交換に使用される技術的なソリューションにより、電子インボイスが拒否される技術的な理由はさまざま（メールボックスが閉じている、構文エラーなど）。  

#### 3.8.6.2 Invoice retransmitted ( 3.8.6.2 電子インボイスの再送 )

E-Invoices should normally only be re-transmitted because for technical reasons the originally issued invoice was not received by the intended recipient. Before re-transmitting, the invoice issuer should verify nonreceipt with the recipient. There should be a documented process for verifying non-receipt and evidence of the verification should be stored in a way to link it back to the relevant invoice.  

技術的な理由により、最初に発行された電子インボイスが目的の受信者に届かなかったとき、通常、電子インボイスは再送信するだけである。 再送信する前に、電子インボイス発行者は、受信者の不着を確認する必要がある。 領収書の不着を検証するための文書化されたプロセスがなければならず、検証の証拠は、それを関連する電子インボイスにリンクする方法で保存する必要がある。  

Invoice recipients should have controls in place to prevent processing of more than one instance of a particular invoice.  

電子インボイスの受信者は、特定の電子インボイスの重複して処理を防止するための適切な管理を行う必要がある。  

### 3.8.7 Format conversion of the E-Invoice ( 3.8.7 電子インボイスの形式変換 )
...
バックエンドシステムで自動データ処理するために、形式変換が必要な場合がある。 製品、売り手、または買い手のコードの内容を変更するなど、内容を変換する必要がある場合がある。 一部の税務当局が、「電子インボイス」を発行されたものと「意味的および構文的に同一」と解釈する限り、元の電子インボイスの発行前または発行後の変換を区別することが重要。  
...
* After having issued the E‐Invoice In all cases  
...
* 電子インボイスを発行する前
  - 電子インボイスデータの変換が可能（現時点では、電子インボイスデータのみが存在）
  - エンドツーエンドの監査証跡を保持することを推奨する

* 電子インボイスの発行後すべての場合
  - 実施された変換の監査証跡を維持することは良い習慣かもしれない
  - 売り手は発行済みの電子インボイスを保管する必要があり、買い手は電子インボイスを受け取ったまま保管する必要がある。 保存は、それぞれのサービスプロバイダーが実行できる。 完全性と真正性は、実装に従って検証可能になる。
...
|  |  |
| ---- | ---- |
| クラス A | 買い手と売り手が実施する独立した真正性と完全性の検証プロセスにより、変換が正確に行われたことが保証される。 |
| クラス B | EDI交換契約には、電子インボイスに適用される変換の詳細が記載されている。 変換によって、電子インボイスの内容の固有の手段が変更されないようにする必要がある。 証拠は、真正性と完全性が維持されていることを示すのに十分でなければならない。 |
| クラス C | 取引相手と介在するサービスプロバイダの間には、どの変換を実行するか、誰が変換するか、変換後の電子インボイスに署名をどのように適用するかについての明確な合意が必要。 署名チェーンの監査証跡を維持する必要がある。 |
...
-
   - 形式および/または内容の変換が送信者および/または受信者で発生する可能性がある。
   - 変換された電子インボイスは、受け取った電子インボイスと常に区別できる必要がある。

In both cases the conversion must not change the E-Invoice semantics and should be reproducible. It is also required to provide tax authorities an access to the Invoice19. Content conversion requires Trading Partners to pay particular attention to the controls they put in place to avoid errors or misunderstandings.  

どちらの場合も、変換によって電子インボイスnoセマンティクスが変更されてはならず、再現可能でなければならない。 税務当局に電子インボイスへのアクセスを提供することも必要。 内容の変換では、エラーや誤解を回避するために取引き相手が実施する管理に特に注意を払う必要がある。  
...
これに関しては、以下を考慮する必要がある。
- a）交換契約および/またはサービスプロバイダー間での契約は、実行する変換を指定し、変換を実行する当事者を識別し、データ抽出が個別に渡されるかどうかを指定する必要がある。
- b）電子インボイスによって表される情報の完全性と真正性は、エンドツーエンドのチェーン全体で検証可能でなければならない。変換を実行するすべてのプロセスは、箇条3.8.5で説明されているように保証されなければならず、具体的には、データの転送は、その箇条3.8.5.5で説明されているように保護されるべきである。使用されたすべてのマッピングの監査証跡を保持する。これには、行われた変更に関する情報と適用時の情報が含まれる。これにより、入力と出力の電子インボイスデータを比較して、表される情報が箇条3.8.5.2で説明したものと同じであることを実証できる。
- c）契約は、適用される法律と、取引相手とサービスプロバイダーの間の契約上の義務の両方に従って、合意形式の電子インボイスデータと合意形式への自動マップのアーカイブに責任がある当事者を指定するものとする。 交換協定は、監査可能な方法でエンドツーエンドチェーンの各ステップを実行する責任がある当事者を指定するものとする。当事者は、どの形式とプロトコルが送信され、どちらが合意形式になるかについて合意する必要がある。

## 3.9 How to use the Compliance Matrix and Interactive User Interface
## 3.9コンプライアンスマトリックスとインタラクティブユーザーインターフェイスの使用方法
...
4. European Commision. Commission Recommendation 94/820/EC relating to the legal aspects of electronic data interchange. [Online] 19 October 1994. [Cited: 25 September 2011.] http://eurlex.europa.eu/LexUriServ/LexUriServ.do?uri=CELEX:31994H0820:en:HTML
...
6. European Telecommunications Standards Institute. ETSI TS 101 862: Qualified certificate profile. [Online] v1.3.2, June 2004. [Cited: 22 9 2011.]
http://www.etsi.org/deliver/etsi_ts/101800_101899/101862/01.03.02_60/ts_101862v010302p.pdf
7. —. ETSI TS 101 456: Electronic Signatures and Infrastructures (ESI); Policy requirements for certification
authorities issuing qualified certificates. [Online] v1.4.3, May 2007.
http://www.etsi.org/deliver/etsi_ts/101400_101499/101456/01.04.03_60/ts_101456v010403p.pdf
8. International Organization for Standardization. ISO/IEC 9594-8: Information technology - Open Systems Interconnection - The Directory: Public-key and attribute certificate frameworks. [Online] 2005. [Cited: 25 September 2011.] http://www.iso.org/iso/catalogue_detail.htm?csnumber=34551
9. —. ISO/IEC 15408 (multipart): Information technology - Security techniques - Evaluation criteria for IT security. [Online] 2009.
10. CEN. CWA 14169: Secure signature-creation devices “EAL 4+”. [Online] March 2004. [Cited: 25 September 2011.] ftp://ftp.cenorm.be/PUBLIC/CWAs/e-Europe/eSign/cwa14169-00-2004-Mar.pdf
11. International Federation of Accountants. ISAE 3402: International Standard on Assurance Engagement. [Online] June 2011. [Cited: 25 September 2011.] http://www.ifac.org/sites/default/files/downloads/b014-2010-iaasb-handbook-isae-3402.pdf
...
20. —. ISO/IEC 27002: Information technology - Security techniqiues - Code of practice for information security management.
21. European Telecommunications Standards Institute. ETSI TS 102 573: Electronic Signatures and Infrastructures (ESI); Policy requirements for trust service providers signing and/or storing data for digital accounting. [Online] v1.1.1, July 2007. [Cited: 22 September 2011.] http://www.etsi.org/deliver/etsi_ts/102500_102599/102573/01.01.01_60/ts_102573v010101p.pdf
22. —. ETSI TS 101 533-01: Electronic Signatures and Infrastructures (ESI); Information Preservation Systems Security - Part 1: Requirements for Implementation and Management. [Online] v1.1.1, June 2008. [Cited: 26 September 2011.] http://www.etsi.org/deliver/etsi_ts/102500_102599/102533/01.01.01_60/ts_102533v010101p.pdf
...
25. European Commision. Commission Decision 2009/767/EC setting out measures facilitating the use of procedures by electronic means through the ‘points of single contact’ under Directive 2006/123/EC of the European Parliament and of the Council on services in the internal market. [Online] 16 October 2009. [Cited:
25. September 2011.] http://eurlex. europa.eu/LexUriServ/LexUriServ.do?uri=OJ:L:2009:299:0018:0054:EN:PDF
26. European Telecommunications Standards Institute. ETSI TS 102 231: Electronic Signatures and Infrastructures (ESI); Provision of harmonized Trust-service status information. [Online] v3.1.2, December
2009. [Cited: 26 September 2011.] http://www.etsi.org/deliver/etsi_ts/102200_102299/102231/03.01.02_60/ts_102231v030102p.pdf
...
28. European Telecommunications Standards Institute. ETSI TS 101 861: Time stamping profile. [Online] March 2002. [Cited: 22 September 2011.] http://docbox.etsi.org/EC_Files/EC_Files/ts_101861v010201p.pdf
29. —. ETSI TS 101 733: Electronic Signatures and Infrastructures (ESI); CMS Advanced Electronic Signatures (CAdES). [Online] January 2011. [Cited: September 22, 2011.] http://www.cryptopro.ru/sites/default/files/products/tsp/ts_101733v010803p.pdf
30. Internet Engineering Task Force. IETF RFC 3852: Cryptographic Message Syntax (CMS). [Online] July
2004. [Cited: 22 September 2011.] http://www.ietf.org/rfc/rfc3852.txt
31. European Telecommunications Standards Institute. ETSI TS 102 734: Electronic Signatures and Infrastructures; Profiles of CMS Advanced Electronic Signatures based on TS 101 733 (CAdES). [Online] v1.1.1, February 2007. [Cited: 22 September 2011.] http://www.etsi.org/deliver/etsi_ts/102700_102799/102734/01.01.01_60/ts_102734v010101p.pdf
32. —. ETSI TS 101 903: XML Advanced Electronic Signature. [Online] v1.4.1, June 2009. [Cited: 22 September 2011.] http://uri.etsi.org/01903/v1.4.1/ts_101903v010401p.pdf
33. W3C. W3C Recommendation: XML Signature Syntax and Processing. [Online] June 2008. [Cited: 25 September 2011.] http://www.w3.org/TR/xmldsig-core/.
34. European Telecommunications Standards Institute. ETSI TS 102 904: Electronic Signatures and Infrastructures; Profiles of XML Advanced Electronic Signatures based on TS 101 903 (XAdES). [Online] v1.1.1, February 2007. [Cited: 22 September 2011.] http://www.etsi.org/deliver/etsi_ts/102900_102999/102904/01.01.01_60/ts_102904v010101p.pdf
...
40. European Telecommunications Standards Institute. ETSI TS 102 640 (multipart): Electronic Signatures and Infrastructures (ESI); Registered Electronic Mail (REM); Architecture, Formats and Policies. [Online] October 2008. [Cited: 25 September 2011.] http://www.eimza.com.tr/dokuman/ts_10264001v010101p.pdf
...
44. European Commission Expert Group on e-invoicing. Final Report of the Expert Group on e-Invoicing.[Online] November 2009. [Cited: September 25, 2011.] http://ec.europa.eu/internal_market/consultations/docs/2009/e-invoicing/report_en.pdf
45. European Telecommunications Standards Institute. ETSI TS 102 778-1: Electronic Signatures and Infrastructures (ESI); PDF Advanced Electronic Signature Profiles; Part 1: PAdES Overview - a framework document for PAdES. [Online] v1.1.1, July 2009. [Cited: 22 September 2011.] http://www.etsi.org/deliver/etsi_ts/102700_102799/10277801/01.01.01_60/ts_10277801v010101p.pdf
46. —. ETSI TS 102 778-2: Electronic Signatures and Infrastructures (ESI); PDF Advanced Electronic Signature Profiles; Part 2: PAdES Basic - Profile based on ISO 32000-1. [Online] July 2009. [Cited: 27 September 2011.] http://www.etsi.org/deliver/etsi_ts/102700_102799/10277802/01.02.01_60/ts_10277802v010201p.pdf
47. —. ETSI TS 102 778-4: Electronic Signatures and Infrastructures (ESI); PDF Advanced Electronic Signature Profiles; Part 4: PAdES Long Term - PAdES-LTV Profile. [Online] December 2009. [Cited: 27 September 2011.] http://www.etsi.org/deliver/etsi_ts/102700_102799/10277804/01.01.02_60/ts_10277804v010102p.pdf
48. —. ETSI TS 102 778-5: Electronic Signatures and Infrastructures (ESI); PDF Advanced Electronic Signature Profiles; Part 5: PAdES for XML Content - Profiles for XAdES signatures. [Online] December 2009.[Cited: 27 September 2011.] http://www.etsi.org/deliver/etsi_ts/102700_102799/10277805/01.01.02_60/ts_10277805v010102p.pdf
49. —. ETSI TS 102 778-3: Electronic Signatures and Infrastructures (ESI); PDF Advanced Electronic Signature Profiles; Part 3: PAdES Enhanced - PAdES-BES and PAdES-EPES Profiles. [Online] July 2010.[Cited: 27 September 2011.]http://www.etsi.org/deliver/etsi_ts/102700_10279910277803/01.02.01_60/ts_10277803v010201p.pdf
