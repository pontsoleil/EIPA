...
```
## preprocess first `mvn -f pom-preprocess.xml generate-resources`
```
...
```
## convert to XSLT - takes forever `mvn -f pom-xslt.xml process-resources`
```
...
[ec2-user@ip-xx-x-x-xxx eInvoicing-EN16931-validation-1.3.2]$ 
```
## Add license headers to all relevant files `mvn -f pom-license.xml license:format`
```
...
[ec2-user@ip-xx-x-x-xxx eInvoicing-EN16931-validation-1.3.2]$
```
## validate afterwards `mvn -f pom-validate.xml validate`
```
