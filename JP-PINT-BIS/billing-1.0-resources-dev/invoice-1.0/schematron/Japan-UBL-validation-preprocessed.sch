...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <!-- Parameters -->
  <let name="profileInvoice" value="
    if (//cbc:ProfileID and //cbc:ProfileID/normalize-space(.)='urn:peppol:pint:billing-3.0@jp:peppol-1')
    then 1 else 0"/>
  <let name="profileJP" value="
    if (//cbc:ProfileID and contains(normalize-space(//cbc:ProfileID), 'urn:peppol:pint:billing-3.0@jp:peppol-1'))
    then 1
    else 0"/>
  ...
  <pattern id="UBL-model">
    <rule context="/ubl:Invoice[cac:ProfileID=@profileInvoice]/cac:AccountingSupplierParty/cac:Party/cac:PartyTaxScheme[cac:TaxScheme/cbc:ID/normalize-space(.)='VAT']">
      <assert id="jp-br-01" flag="fatal" test="matches(cbc:CompanyID/normalize-space(.),'^T[0-9]{13}$')">[jp-br-01]- For the Japanese Suppliers, the Tax identifier must start with 'T' and must be 13 digits.</assert>
    ...
    <rule context="/*/cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory[cbc:ID/normalize-space(.)='S'][cac:TaxScheme/cbc:ID/normalize-space(.)='VAT']">
	...
	<rule context="/*/cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory[cbc:ID/normalize-space(.)='AA'][cac:TaxScheme/cbc:ID/normalize-space(.)='VAT']">
    ...
    <rule context="cac:LegalMonetaryTotal">
		...
		<let name="payableAmount" value="
 		...
 		<let name="allowancesTotal" value="
			if (cbc:allowancesTotal) 
			then cbc:allowancesTotal/xs:decimal(.)
			else 0"/>
		<let name="chargesTotal" value="
			if (cbc:chargesTotal) 
			then cbc:chargesTotal/xs:decimal(.)
			else 0"/>
		...
		<assert id="ibr-co-10" flag="fatal" test="($lineExtensionAmount = sum(cbc:chargesTotal/xs:decimal(.)))">
      [ibr-co-10]-Sum of Invoice line net amount (IBT-106) = Σ Invoice line net amount (IBT-131).
		...
		<assert id="ibr-co-13" flag="fatal" test="$taxExclusiveAmount = $chargesTotal - $allowancesTotal + $chargesTotal">
