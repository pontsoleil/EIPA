<?xml version="1.0" encoding="UTF-8"?><schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <pattern id="UBL-model">
    <rule context="/ubl:Invoice[cac:AccountingSupplierParty/cac:Party/cac:PostalAddress/cac:Country/cbc:IdentificationCode='JP' ]/cac:AccountingSupplierParty/cac:Party/cac:PartyTaxScheme[cac:TaxScheme/cbc:ID/normalize-space(.)='VAT']">
      ...
      <assert id="jp-s-02" flag="fatal" test="(exists(//cac:ClassifiedTaxCategory[cbc:ID/normalize-space(.)='S'][cac:TaxScheme/cbc:ID/normalize-space(.)='VAT']) and exists(//cac:AccountingSupplierParty/cac:Party/cac:PartyTaxScheme/cbc:CompanyID))">[jp-s-02]-An Invoice that contains an Invoice line (IBG-25) where the Invoiced item Consumption Tax category code (IBT-151) is "Standard rated" shall contain the Seller Consumption Tax Identifier (IBT-31), the Seller tax registration identifier (IBT-32) and/or the Seller tax representative Consumption Tax identifier (IBT-63).</assert>
      <assert id="jp-s-02aa" flag="fatal" test="(exists(//cac:ClassifiedTaxCategory[cbc:ID/normalize-space(.)='AA'][cac:TaxScheme/cbc:ID/normalize-space(.)='VAT']) and (exists(//cac:AccountingSupplierParty/cac:Party/cac:PartyTaxScheme/cbc:CompanyID) or exists(//cac:TaxRepresentativeParty/cac:PartyTaxScheme[cac:TaxScheme/(cbc:ID/normalize-space(.)='VAT')]/cbc:CompanyID))) or not(exists(//cac:ClassifiedTaxCategory[cbc:ID/normalize-space(.)='S']))">[jp-s-02]-An Invoice that contains an Invoice line (IBG-25) where the Invoiced item Consumption Tax category code (IBT-151) is "Standard rated" shall contain the Seller Consumption Tax Identifier (IBT-31), the Seller tax registration identifier (IBT-32) and/or the Seller tax representative Consumption Tax identifier (IBT-63).</assert>
      <assert id="jp-s-03" flag="fatal" test="(exists(//cac:AllowanceCharge[cbc:ChargeIndicator=false()]/cac:TaxCategory[cbc:ID/normalize-space(.)='S'][cac:TaxScheme/cbc:ID/normalize-space(.)='VAT']) and (exists(//cac:AccountingSupplierParty/cac:Party/cac:PartyTaxScheme/cbc:CompanyID) or exists(//cac:TaxRepresentativeParty/cac:PartyTaxScheme[cac:TaxScheme/(cbc:ID/normalize-space(.)='VAT')]/cbc:CompanyID))) or not(exists(//cac:AllowanceCharge[cbc:ChargeIndicator=false()]/cac:TaxCategory[cbc:ID/normalize-space(.)='S'][cac:TaxScheme/cbc:ID/normalize-space(.)='VAT']))">[jp-s-03]-An Invoice that contains a Document level allowance (IBG-20) where the Document level allowance Consumption Tax category code (IBT-95) is "Standard rated" shall contain the Seller Consumption Tax Identifier (IBT-31), the Seller tax registration identifier (IBT-32) and/or the Seller tax representative Consumption Tax identifier (IBT-63).</assert>
      <assert id="jp-s-04" flag="fatal" test="(exists(//cac:AllowanceCharge[cbc:ChargeIndicator=true()]/cac:TaxCategory[cbc:ID/normalize-space(.)='S'][cac:TaxScheme/cbc:ID/normalize-space(.)='VAT']) and (exists(//cac:AccountingSupplierParty/cac:Party/cac:PartyTaxScheme/cbc:CompanyID) or exists(//cac:TaxRepresentativeParty/cac:PartyTaxScheme[cac:TaxScheme/(cbc:ID/normalize-space(.)='VAT')]/cbc:CompanyID))) or not(exists(//cac:AllowanceCharge[cbc:ChargeIndicator=true()]/cac:TaxCategory[cbc:ID/normalize-space(.)='S'][cac:TaxScheme/cbc:ID/normalize-space(.)='VAT']))">[jp-s-04]-An Invoice that contains a Document level charge (IBG-21) where the Document level charge Consumption Tax category code (IBT-102) is "Standard rated" shall contain the Seller Consumption Tax Identifier (IBT-31), the Seller tax registration identifier (IBT-32) and/or the Seller tax representative Consumption Tax identifier (IBT-63).</assert>
    </rule>
    <rule context="/ubl:Invoice[cac:AccountingSupplierParty/cac:Party/cac:PostalAddress/cac:Country/cbc:IdentificationCode='JP' ]">
      <assert id="jp-br-co-01" flag="fatal" test="(round(cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory/cbc:Percent/xs:decimal(.)) != 0 and (     xs:decimal(cac:TaxTotal/cac:TaxSubtotal/cbc:TaxAmount) &gt;= floor(xs:decimal(cac:TaxTotal/cac:TaxSubtotal/cbc:TaxableAmount) * (cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory/cbc:Percent/xs:decimal(.) div 100)))     and (     xs:decimal(cac:TaxTotal/cac:TaxSubtotal/cbc:TaxAmount) &lt;= ceiling(xs:decimal(cac:TaxTotal/cac:TaxSubtotal/cbc:TaxableAmount) * (cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory/cbc:Percent/xs:decimal(.) div 100)))     )">[jp-br-co-01]-Consumption Tax category tax amount (IBT-117) = Consumption Tax category taxable amount (IBT-116) x (Consumption Tax category rate (IBT-119) / 100), rounded to integer. The rounded result amount SHALL be between the floor and the ceiling.</assert>
    </rule>
    ...
    <rule context="cac:AllowanceCharge[cbc:ChargeIndicator=false()]/cac:TaxCategory[cbc:ID/normalize-space(.)='S' or cbc:ID/normalize-space(.)='AA'][cac:TaxScheme/cbc:ID/normalize-space(.)='VAT']">
    ...
    <rule context="/*/cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory[cac:TaxScheme/cbc:ID/normalize-space(.)='VAT']">
      ...
      <assert id="jp-s-08s" flag="fatal" test="(
          ...
          ../cbc:TaxableAmount/xs:decimal(.) = sum($invoiceLineStandard/cbc:LineExtensionAmount/xs:decimal(.)) +
          sum($chargeStandard/cbc:Amount/xs:decimal(.)) - sum($allowanceStandard/cbc:Amount/xs:decimal(.))
        )">
  ...
  <pattern id="Codesmodel">
    ...
    <rule flag="fatal" context="cbc:TaxExemptionReasonCode">
      <assert id="jp-cl-04" flag="fatal" test="((not(contains(normalize-space(.), ' ')) and contains(' ZZZ ', concat(' ', normalize-space(.), ' '))))">[jp-cl-04]-Tax exemption reason code identifier scheme identifier MUST belong to the ????</assert>
    </rule>
  </pattern>
</schema>